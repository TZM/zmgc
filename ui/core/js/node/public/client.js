function ZmgcClient() {
  ...
  this.drawGlobe = function() {
    var width = height = $("#viz").width(),
        centered, feature, lat = lon = 0.0;

    var projection = d3.geo.azimuthal().scale(95).origin([lat, lon]).mode("orthographic").translate([width - 100, height - 80]);
    ...
    var svg = d3.select("#viz").append("svg:svg").attr("width", width).attr("height", height).on("mousedown", mousedown);


    svg.append("rect").attr("fill", "none").attr("width", width).attr("height", height).on("mouseover", function() {
    ...
    d3.json("/world-countries.json", function(collection) {
      feature = svg.selectAll("path").data(collection.features).enter().append("svg:path").on("mouseover", function(d) {
        d3.select(this).style("fill", "#6C0");
      }).on("mouseout", function(d) {
        d3.select(this).style("fill", "#000000");
      }).on("click", click).attr("d", clip);
  ...
  this.drawMap = function() {
    ...
    self.svg = d3.select("#map").append("svg:svg").attr("width", "100%").attr("height", "87%").attr("viewBox", "0 0 1024 768");

    self.map = d3.geo.equirectangular().scale(width).translate([500, 400]);
  ...
  this.enlargeMarker = function mouseover(d) {
    ...
    d3.select(this).transition().duration(750).attr("transform", "translate(480,480)scale(23)rotate(180)").transition().delay(1500).attr("transform", "translate(240,240)scale(10)").style("fill-opacity", 10)
    //.remove();
  }
  // Initialise
  this.init();
